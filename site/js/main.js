/* BurkeBuild - interactions (shared across all pages) */
(function () {
  'use strict';
  ...
  /* ----- lightbox (case-study cards navigate instead) ----- */
  ...
  if (lightbox) {
    ...
    const visibleProjects = () =>
      Array.from(projects).filter(
        (p) => !p.classList.contains('is-hidden') && !('case' in p.dataset)
      );
    ...
    projects.forEach((p) => {
      p.addEventListener('click', (e) => {
        if ('case' in p.dataset) return; // real link to a case-study page
        e.preventDefault();
